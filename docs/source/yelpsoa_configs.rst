...
instance MAY have:
  ...
  * ``bounce_margin_factor``: proportionally increase the number of old instances
    ...
    0 < bounce_margin_factor <= 1. Defaults to 0.95 (kill up to 5% of replicas or allow the bounce to be considered finished even if up to 5% of replicas are unhealthy).
