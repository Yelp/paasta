...
specify the following options:
  ...
  * ``env``: A dictionary of environment variables that will be made available
    to the container. Values can be plain strings or use the ``SECRET()`` macro
    to inject secrets from the service's ``secrets/`` directory. For example::

      env:
        MY_SECRET_VAR: SECRET(test-secret)
        PAASTA_SECRET_MYSQL_VAR: SECRET(mysql_credentials)
        REGULAR_VAR: "plain-value"

    The ``SECRET()`` macro references secrets that are stored encrypted in
    yelpsoa-configs and decrypted by Vault. The secret name corresponds to the
    filename in the service's ``secrets/`` directory without the ``.json`` suffix
    (e.g., ``SECRET(test-secret)`` references ``secrets/test-secret.json``).
    Secrets are managed by the ``paasta secret`` command. For more details, see
    `PaaSTA service secrets <http://y/paasta-secrets-docs>`_.

    Secrets can also be injected as files. See ``secret_volumes``.

    PaaSTA additionally will inject the following variables automatically (keep in mind all environment variables are strings in a shell):
  ...
  * ``secret_volumes``: An array of dictionaries for mounting secrets as files
    in the container. Secrets are stored encrypted in yelpsoa-configs and
    decrypted by Vault. Secrets are managed by the ``paasta secret`` command.
    There are two ways to configure secret volumes:

    **Simple format** (secret name becomes the filename within the directory)::
    ...
    This creates files at ``/nail/foo/the_secret_1`` and ``/nail/bar/the_secret_2``.
    The ``container_path`` is the directory, and ``secret_name`` becomes the filename.

    **Items format** (explicit filenames for multiple secrets in one directory)::
    ...
    This creates files at ``/nail/foo/bar.yaml`` and ``/nail/foo/baz.yaml``.

    The simple format example above is equivalent to::
    ...
    Each secret volume dictionary must contain ``container_path`` and either
    ``secret_name`` OR ``items``:

    * ``container_path``: The directory in the container where the secret file(s)
      will be mounted. (Required for both formats)
    * ``secret_name``: (Simple format only) The name of the secret, corresponding
      to the filename in ``secrets/`` without the ``.json`` suffix (e.g.,
      ``test-secret`` references ``secrets/test-secret.json``). The file will be
      created with this name. This field is ignored if ``items`` is specified.
    * ``items``: (Items format only) A list of secrets with custom filenames.
      If specified, ``secret_name`` is ignored:

      * ``key``: The name of the secret, corresponding to the filename in
        ``secrets/`` without the ``.json`` suffix.
      * ``path``: The filename to use in the container.

    For more details, see `PaaSTA service secrets <http://y/paasta-secrets-docs>`_.
...
instance MAY have:
  ...
  * ``autoscaling``: See the `autoscaling docs <autoscaling.html>`_ for details
    ...
    * ``metrics_providers``: A list of data sources to use for autoscaling:
        ...
        * ``type``: Which method the autoscaler will use to determine a service's utilization.
          Should be ``cpu``, ``worker-load``, ``uwsgi``, ``active-requests``, ``piscina``, ``gunicorn``, or ``arbitrary-promql``.
  ...
  * ``preemptible``: A boolean indicating whether this instance should run at low priority and be freely evictable.
    Defaults to ``false``. When ``true``, a PriorityClass with value -1000 and ``preemptionPolicy: PreemptLowerPriority``
    is created, and the PodDisruptionBudget is set to ``maxUnavailable: 100%`` so the pods can always be evicted.
