...
async def kubernetes_status_v2(
    ...
    if (
        ...
        tasks.append(autoscaling_task)  # type: ignore  # PAASTA-18698; ignoring due to type mismatch
    else:
    ...
    tasks.append(pods_task)  # type: ignore  # PAASTA-18698; ignoring due to type mismatch
    ...
    if "proxy_port" in service_namespace_config:
        ...
        backends_task = asyncio.create_task(
            get_backends_from_mesh_status(mesh_status_task)  # type: ignore  # PAASTA-18698; ignoring due to type mismatch
        )
        tasks.extend([mesh_status_task, backends_task])  # type: ignore  # PAASTA-18698; ignoring due to type mismatch
    else:
    ...
    if job_config.get_persistent_volumes():
        pod_status_by_sha_and_readiness_task = asyncio.create_task(
            get_pod_status_tasks_by_sha_and_readiness(
                pods_task,
                backends_task,  # type: ignore  # PAASTA-18698; ignoring due to type mismatch
                kube_client,
                verbose,
            )
        )
        versions_task = asyncio.create_task(
            get_versions_for_controller_revisions(
                ...
                instance=instance,
                namespaces=relevant_namespaces,
                pod_status_by_sha_and_readiness_task=pod_status_by_sha_and_readiness_task,  # type: ignore  # PAASTA-18698; ignoring due to type mismatch
            )
        )
        tasks.extend([pod_status_by_sha_and_readiness_task, versions_task])  # type: ignore  # PAASTA-18698; ignoring due to type mismatch
    else:
        ...
        versions_task = asyncio.create_task(
            get_versions_for_replicasets(
                ...
                instance=instance,
                namespaces=relevant_namespaces,
                pod_status_by_replicaset_task=pod_status_by_replicaset_task,  # type: ignore  # PAASTA-18698; ignoring due to type mismatch
            )
        )
        tasks.extend([pod_status_by_replicaset_task, versions_task])  # type: ignore  # PAASTA-18698; ignoring due to type mismatch
