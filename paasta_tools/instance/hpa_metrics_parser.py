class HPAMetricsParser():
    ...
    def parse_external_metric(self, metric_spec, status):
        status[self.NAME] = metric_spec.metric_name
        status[self.TARGET] = metric_spec.target_average_value if hasattr(metric_spec, "target_average_value") else metric_spec.target_value

    def parse_external_metric_current(self, metric_spec, status):
        status[self.NAME] = metric_spec.metric_name
        status[self.CURRENT] =metric_spec.current_average_value if hasattr(metric_spec, "current_average_value") else metric_spec.current_value
    ...
    def parse_resource_metric(self, metric_spec, status):
        ...
        status[self.TARGET] = metric_spec.target_average_value if hasattr(metric_spec, "target_average_value") else metric_spec.target_average_utilization

    def parse_resource_metric_current(self, metric_spec, status):
        status[self.NAME] = metric_spec.name
        status[self.CURRENT] =metric_spec.current_average_value if hasattr(metric_spec, "current_average_value") else metric_spec.current_average_utilization
