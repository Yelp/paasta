...
async def format_stdstreams_tail_for_task(task, get_short_task_id, nlines=10):
    tail_lines_dict = await get_tail_lines_for_mesos_task(task, get_short_task_id, nlines)
    ...
    return [f'    {line}' for line in format_tail_lines_for_mesos_task(tail_lines, task['id'])]
