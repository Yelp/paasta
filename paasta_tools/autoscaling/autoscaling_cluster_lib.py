...
from paasta_tools.utils import ClusterAutoscalingResource
from paasta_tools.utils import IdToClusterAutoscalingResourcesDict
from paasta_tools.utils import load_system_paasta_config
from paasta_tools.utils import PoolToResourcePoolSettingsDict
from paasta_tools.utils import ResourcePoolSettings
...
class SpotFleetRequestConfig(TypedDict):
    ...
    TargetCapacity: int
    LaunchSpecifications: Sequence[LaunchSpecification]
...
MISSING_SLAVE_PANIC_THRESHOLD = 0.3
...
class ClusterAutoscaler:
    def __init__(
        ...
        pool_settings: ResourcePoolSettings,
        config_folder: str,
        dry_run: bool,
        ...
        self.instances: Sequence[InstanceDescription] = []
        self.sfr: Optional[SpotFleetRequestConfig0] = None
        self.enable_metrics = enable_metrics
        self.enable_maintenance_reservation = enable_maintenance_reservation

        self.log.info(f"Initialized with utilization error {self.utilization_error}")
        config = load_system_paasta_config()
        self.slave_newness_threshold = config.get_monitoring_config().get(
    ...
    def emit_metrics(
        self, current_capacity: float, target_capacity: float, mesos_slave_count: int
    ) -> None:
        if not self.enable_metrics:
            return None
    ...
    def describe_instances(
        ...
        try:
            instance_descriptions = ec2_client.describe_instances(
                InstanceIds=instance_ids, Filters=instance_filters
            )
        except ClientError as e:
            if e.response["Error"]["Code"] == "InvalidInstanceID.NotFound":
                self.log.warn(
                    f"Cannot find one or more instance from IDs {instance_ids}"
                )
                return None
            else:
                raise
    ...
    def get_instance_ips(
        self, instances: Sequence[InstanceDescription], region: Optional[str] = None
    ) -> Sequence[str]:
        instance_descriptions = self.describe_instances(
            [instance["InstanceId"] for instance in instances], region=region
        )
        instance_ips = []
        for instance in instance_descriptions:
            try:
                ...
                self.log.warning(
                    "Instance {} does not have an IP. This normally means it has been"
                    " terminated".format(instance["InstanceId"])
                )
        return instance_ips

    def cleanup_cancelled_config(
        self, resource_id: str, config_folder: str, dry_run: bool = False
    ) -> None:
        file_name = f"{resource_id}.json"
        configs_to_delete = [
            os.path.join(walk[0], file_name)
            for walk in os.walk(config_folder)
            if file_name in walk[2]
        ]
        ...
        self.log.info(f"Deleted Resource config {configs_to_delete[0]}")

    def get_aws_slaves(self, mesos_state: MesosState) -> Mapping[str, Mapping]:
        instance_ips = self.get_instance_ips(
            self.instances, region=self.resource["region"]
        )
        slaves = {
            slave["id"]: slave
            for slave in mesos_state.get("slaves", [])
            if slave_pid_to_ip(slave["pid"]) in instance_ips
            and slave["attributes"].get("pool", "default") == self.resource["pool"]
        }
        return slaves

    def check_expected_slaves(
        self, slaves: Mapping[str, Mapping], expected_instances: Optional[int]
    ) -> None:
    ...
    async def can_kill(
        self, hostname: str, should_drain: bool, dry_run: bool, timer: Timer
    ) -> bool:
        if dry_run:
            return True
        if timer.ready():
            self.log.warning(
                "Timer expired before slave ready to kill, proceeding to terminate anyways"
            )
            timer.start()
            raise TimeoutError
        if not should_drain:
            self.log.info(
                "Not draining, waiting %s longer before killing" % timer.left()
            )
            return False
        if await a_sync.run(is_safe_to_kill, hostname):
            self.log.info(
                f"Slave {hostname} is ready to kill, with {timer.left()} left on timer"
            )
            timer.start()
            return True
        return False
    ...
    async def wait_and_terminate(
        ...
        """
        self.log.info(
            "Starting TERMINATING: {} (Hostname = {}, IP = {})".format(
                slave.instance_id, slave.hostname, slave.ip
            )
        )
        try:
                ...
                if await self.can_kill(slave.hostname, should_drain, dry_run, timer):
                    self.log.info(
                        "TERMINATING: {} (Hostname = {}, IP = {})".format(
                            instance_id, slave.hostname, slave.ip
                        )
                    )
            ...
            self.log.error(
                "Timed out after {} waiting to drain {}, now terminating anyway".format(
                    timer.timeout, slave.pid
                )
            )
            self.terminate_instances([instance_id])
    ...
    def terminate_instances(self, instance_ids: Sequence[str]) -> None:
        ...
        try:
            ec2_client.terminate_instances(
                InstanceIds=instance_ids, DryRun=self.dry_run
            )
        except ClientError as e:
    ...
    async def scale_resource(
        self, current_capacity: float, target_capacity: int
    ) -> None:
        ...
        if delta == 0:
            ...
            slaves_list = await get_mesos_task_count_by_slave(
                mesos_state, pool=self.resource["pool"]
            )
            filtered_slaves = self.filter_aws_slaves(slaves_list)
            killable_capacity = round(
                sum([slave.instance_weight for slave in filtered_slaves]), 2
            )
            amount_to_decrease = round(delta * -1, 2)
            if amount_to_decrease > killable_capacity:
                instance_ids_in_mesos = {slave.instance_id for slave in filtered_slaves}
    ...
    async def gracefully_terminate_slave(
        self, slave_to_kill: "PaastaAwsSlave", capacity_diff: int, timer: Timer
    ) -> None:
        """
        ...
        if should_drain:
            try:
                ...
                raise
        self.log.info(
            "Decreasing resource from {} to: {}".format(
        ...
        try:
            ...
            self.log.error(
                f"Setting resource capacity back to {(self.capacity - capacity_diff)}"
            )
            self.set_capacity(self.capacity - capacity_diff)
            self.log.info(f"Undraining {slave_to_kill.pid}")
            if should_drain:
    ...
    def filter_aws_slaves(
        self, slaves_list: Sequence[Mapping[str, SlaveTaskCount]]
    ) -> Sequence["PaastaAwsSlave"]:
        ...
        slave_ips = [
            slave_pid_to_ip(slave["task_counts"].slave["pid"]) for slave in slaves
        ]
        ...
        for slave in slaves:
            ...
            if matching_descriptions:
                assert len(matching_descriptions) == 1, (
                    ...
                    % (
                        ",".join([x["InstanceId"] for x in matching_descriptions]),
                        slave_ip,
                    )
                )
                ...
                ), "There should be only one InstanceStatus per instance"
            else:
                description = None
    ...
    def instance_status_for_instance_ids(
        self, instance_ids: Sequence[str]
        ...
        partitions = [
            ...
            for partition in range(0, len(instance_ids), 99)
        ]
        ...
        for subgroup in partitions:
            res = self.describe_instance_status(
                instance_ids=subgroup, region=self.resource["region"]
            )
            accumulated["InstanceStatuses"] += res["InstanceStatuses"]

        return accumulated

    def instance_descriptions_for_ips(
        self, ips: Sequence[str]
    ) -> Sequence[InstanceDescription]:
        all_instances: List[InstanceDescription] = []
        for start, stop in zip(
            range(0, len(ips), 199), range(199, len(ips) + 199, 199)
        ):
            all_instances += self.describe_instances(
                instance_ids=[],
                region=self.resource["region"],
                instance_filters=[
                    {"Name": "private-ip-address", "Values": ips[start:stop]}
                ],
            )
        return all_instances

    def filter_instance_description_for_ip(
        self, ip: str, instance_descriptions: Sequence[InstanceDescription]
    ) -> Sequence[InstanceDescription]:
        return [i for i in instance_descriptions if ip == i["PrivateIpAddress"]]

    def filter_instance_status_for_instance_id(
        self,
        instance_id: str,
        instance_statuses: Mapping[
            str, Sequence[Mapping[str, Union[str, Mapping, Sequence]]]
        ],
    ...
    async def downscale_aws_resource(
        ...
        self.log.info(f"downscale_aws_resource for {filtered_slaves}")
        killed_slaves = 0
        terminate_tasks = {}
        self.capacity = current_capacity
        timer = Timer(300)
        while True:
            filtered_sorted_slaves = ec2_fitness.sort_by_ec2_fitness(filtered_slaves)[
                ::-1
            ]
            if len(filtered_sorted_slaves) == 0:
                ...
                break
            self.log.info(
                "Resource slave kill preference: {}".format(
            ...
            if new_capacity < target_capacity:
                ...
                if (
                    self.resource["type"] == "aws_spot_fleet_request"
                    and killed_slaves == 0
                ):
                    self.log.info(
            ...
            self.log.info(f"Starting async kill for {slave_to_kill.hostname}")
...
class SpotAutoscaler(ClusterAutoscaler):
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        ...
        self.sfr: SpotFleetRequestConfig0 = self.get_sfr(
            self.resource["id"], region=self.resource["region"]
        )
        if self.sfr:
            self.instances = self.get_spot_fleet_instances(
                self.resource["id"], region=self.resource["region"]
            )
            if (
                self.sfr["SpotFleetRequestState"] == "cancelled_running"
                and self.utilization_error < 0
            ):
                self.log.warn(
    ...
    def get_sfr(
        self, spotfleet_request_id: str, region: Optional[str] = None
    ) -> Optional[SpotFleetRequestConfig0]:
        ec2_client = boto3.client("ec2", region_name=region)
        try:
            sfrs = ec2_client.describe_spot_fleet_requests(
                SpotFleetRequestIds=[spotfleet_request_id]
            )
        except ClientError as e:
    ...
    def is_new_autoscaling_resource(self) -> bool:
        ...
        """
        if not self.sfr:
            self.log.warn(f"Cannot find SFR {self.resource['id']}")
            return True

        now = datetime.now(timezone.utc)
        return (
            now - self.sfr["CreateTime"]
        ).total_seconds() < self.slave_newness_threshold
    ...
    def metrics_provider(self, mesos_state: MesosState) -> Tuple[float, int]:
        if not self.sfr or self.sfr["SpotFleetRequestState"] == "cancelled":
            ...
            self.cleanup_cancelled_config(
                self.resource["id"], self.config_folder, dry_run=self.dry_run
            )
            return 0, 0
        elif (
            self.sfr["SpotFleetRequestState"] == "cancelled_running"
            and len(self.instances) == 0
        ):
            self.log.error(
            ...
            self.log.error(
                "Unexpected SFR state: {} for {}".format(
                    self.sfr["SpotFleetRequestState"], self.resource["id"]
                )
            )
    ...
    def get_spot_fleet_delta(self) -> Tuple[float, int]:
        ...
        self.log.debug(
            f"The ideal capacity to scale to is {self.ideal_capacity:d} instances"
        )
        self.log.debug(f"The capacity we will scale to is {new_capacity:d} instances")
    ...
    def set_capacity(self, capacity: float) -> Optional[Any]:
        ...
        with Timeout(seconds=AWS_SPOT_MODIFY_TIMEOUT):
            try:
                ...
                self.log.error(
                    "Spot fleet {} not in active state so we can't modify it.".format(
                        self.resource["id"]
                    )
                )
    ...
    def get_instance_type_weights(self) -> Mapping[str, float]:
        launch_specifications = self.sfr["SpotFleetRequestConfig"][
            "LaunchSpecifications"
        ]
        return {
            ls["InstanceType"]: ls["WeightedCapacity"] for ls in launch_specifications
        }
...
class AsgAutoscaler(ClusterAutoscaler):
    ...
    def is_new_autoscaling_resource(self) -> bool:
        ...
        if not self.asg:
            self.log.warning(
                f"ASG {self.resource['id']} not found, removing config file"
            )
            return True

        now = datetime.now(timezone.utc)
        return (
            now - self.asg["CreatedTime"]
        ).total_seconds() < self.slave_newness_threshold

    def get_asg(
        self, asg_name: str, region: Optional[str] = None
    ...
    def get_ideal_capacity(self) -> int:
        if self.current_capacity == 0:
            return int(
                min(
                    max(1, self.resource["min_capacity"]), self.resource["max_capacity"]
                )
            )
        else:
            return int(ceil((1 + self.utilization_error) * self.current_capacity))
    ...
    def set_capacity(self, capacity: float) -> Optional[Any]:
        ...
        try:
            ret = asg_client.update_auto_scaling_group(
                AutoScalingGroupName=self.resource["id"], DesiredCapacity=capacity
            )
        except ClientError as e:
...
def get_all_utilization_errors(
    autoscaling_resources: IdToClusterAutoscalingResourcesDict,
    all_pool_settings: PoolToResourcePoolSettingsDict,
    ...
    for identifier, resource in autoscaling_resources.items():
        ...
        ).get("target_utilization", DEFAULT_TARGET_UTILIZATION)
...
async def autoscale_local_cluster(
    config_folder: str, dry_run: bool = False, log_level: str = None
) -> None:
    ...
    autoscaling_draining_enabled = (
        system_config.get_cluster_autoscaling_draining_enabled()
    )
    ...
    log.info(f"Utilization errors: {utilization_errors}")
    autoscaling_scalers: DefaultDict[
        Tuple[str, str], List[ClusterAutoscaler]
    ] = defaultdict(list)
    for identifier, resource in autoscaling_resources.items():
        pool_settings = all_pool_settings.get(
            resource["pool"],
            {
                "target_utilization": DEFAULT_TARGET_UTILIZATION,
                "drain_timeout": DEFAULT_DRAIN_TIMEOUT,
        ...
        try:
            scaler = get_scaler(resource["type"])(
                ...
                log_level=log_level,
                utilization_error=utilization_errors[
        ...
        time.sleep(3)
    filtered_autoscaling_scalers = filter_scalers(
...
def sort_scalers(
    ...
    return sorted(
        ...
        key=lambda x: x.is_resource_cancelled(),
        reverse=True,
    )
...
def filter_scalers(
    ...
    any_cancelled_in_pool: DefaultDict[Tuple[str, str], bool] = defaultdict(
        lambda: False
    )
    ...
    for (region, pool), scalers in autoscaling_scalers.items():
        if (
            any_cancelled_in_pool[(region, pool)]
            and utilization_errors[(region, pool)] < 0
        ):
            filtered_autoscaling_scalers += [
                s for s in scalers if s.is_resource_cancelled()
            ]
            skipped: List[ClusterAutoscaler] = [
                s for s in scalers if not s.is_resource_cancelled()
            ]
            log.info(
                "There are cancelled resources in pool %s, skipping active resources"
                % pool
            )
...
async def run_parallel_scalers(
    sorted_autoscaling_scalers: Sequence[ClusterAutoscaler], mesos_state: MesosState
) -> None:
    scaling_tasks = []
    for scaler in sorted_autoscaling_scalers:
        scaling_tasks.append(
            asyncio.ensure_future(autoscale_cluster_resource(scaler, mesos_state))
        )
        log.debug("Sleep 3s to throttle AWS API calls")
...
def get_scaler(scaler_type: str,) -> Type[ClusterAutoscaler]:
    scalers = {
...
async def autoscale_cluster_resource(
    scaler: ClusterAutoscaler, mesos_state: MesosState
) -> None:
    log.info(f"Autoscaling {scaler.resource['id']} in pool, {scaler.resource['pool']}")
    try:
        ...
        log.error(f"{scaler.resource['id']}: {e}")


def get_instances_from_ip(
    ip: str, instance_descriptions: Sequence[InstanceDescription]
) -> Sequence[InstanceDescription]:
...
def autoscaling_info_for_resource(
    ...
    if not scaler.exists:
        log.info(f"no scaler for resource {resource['id']}. ignoring")
        return None
    target_capacity: Union[int, str]
    try:
    ...
    try:
        region_pool_utilization_dict = get_resource_utilization_by_grouping(
            lambda slave: (
                ...
                ("region", slave["attributes"]["datacenter"]),
            ),
            mesos_state,
