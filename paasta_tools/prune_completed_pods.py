...
def parse_args():
    parser = argparse.ArgumentParser(description="Terminates completed pods based on namespace and minutes since completion")
    parser.add_argument(
        "-n",
        "--namespace",
...
def main():
    ...
    for pod in pods:
        if is_pod_completed(pod):
            if completed_since(pod, allowed_uptime_minutes):
