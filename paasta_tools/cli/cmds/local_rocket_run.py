...
def add_subparser(subparsers):
    list_parser = subparsers.add_parser(
        ...
        description=(
            "'paasta local-rocket-run' is useful for simulating how a PaaSTA service would be "
            "executed on a real cluster -- but using rocket instead of Docker. It analyzes "
            "the local soa-configs and constructs a 'rkt run' invocation to match. This is "
            "useful as a type of end-to-end test, ensuring that a service will work inside "
            "the Docker container on rocket (Docket, if you will) as expected. Additionally, "
            "'local-rocket-run' can heal thcheck a service per the configured healthcheck. "
            "\n\nAlternatively, 'local-rocket-run' can be used with --pull, which will pull "
            "the currently deployed docker image and use it, instead of building one."
        ),
        epilog=(
            "Note: 'paasta local-rocket-run' uses rocket commands, which may require elevated privileges "
            "to run (sudo)."
        ),
    )
    ...
    build_pull_group.add_argument(
        '-d', '--dry-run',
        help='Shows the arguments supplied to rocket as json.',
        action='store_const',
        const='dry_run',
        dest='action',
    )
...
def get_container_name():
    return 'paasta_local_run_{username}_{id}'.format(username=get_username(), id=randint(1, 999999))
...
def get_rkt_run_cmd(
    ...
    for k, v in env.iteritems():
        cmd.append('--set-env=\"%s=%s\"' % (k, v))
    cmd.append('--set-env=MARATHON_PORT=%s' % random_port)
    cmd.append('--set-env=HOST=%s' % hostname)
    cmd.append('--set-env=MESOS_SANDBOX=/mnt/mesos/sandbox')
    cmd.append('%s' % docker_hash)
    cmd.append('--memory=%dM' % memory)
#    for i in rocket_params:
#        cmd.append('--%s=%s' % (i['key'], i['value']))
    if net == 'bridge':
        cmd.append('--port=%d:%d' % (random_port, CONTAINER_PORT))
    elif net == 'host':
        cmd.append('--net=host')
    cmd.append('--hostname=%s' % container_name)
    if interactive:
        cmd.append('--interactive=true')
    else:
        cmd.append('--interactive=false')
    for mount in mounts:
        cmd.append('--mount {}'.format(mount))
    if command:
        cmd.extend((
            'sh', '-c'
        ))
...
def rkt_fetch_image(docker_url):
    """Fetch an image via ``rkt fetch``. Uses the actual fetch command instead of the python
    ...
    ret, output = _run(
        'rkt --insecure-options=image --user-config=/etc/rkt/ fetch docker://{url}'.format(
            url=docker_url),
        stream=True,
        stdin=DEVNULL
    )
...
def run_rkt_container(
    ...
    volume_args, mount_args = docker_volumes_to_rocket_volumes_and_mounts(volumes)
    rocket_run_args = dict(
        memory=memory,
        ...
        hostname=hostname,
        net=net,
        rocket_params=docker_params_to_rocket_params(docker_params),
        mounts=mount_args,
    )
    rocket_run_cmd = get_rkt_run_cmd(**rocket_run_args)
    joined_rocket_run_cmd = ' '.join(rocket_run_cmd)
    ...
    if dry_run:
        if json_dict:
            sys.stdout.write(json.dumps(rocket_run_args) + '\n')
        else:
            sys.stdout.write(json.dumps(rocket_run_cmd) + '\n')
        sys.exit(0)
    else:
        sys.stdout.write('Running docker command:\n%s\n' % PaastaColors.grey(joined_rocket_run_cmd))
    ...
    container_id = None
    try:
        (returncode, output) = _run(joined_rocket_run_cmd)
        if returncode != 0:
            sys.stdout.write(
...
def docker_volumes_to_rocket_volumes_and_mounts(volumes):
    volume_args = []
    mount_args = []
    for volume in volumes:
        src, dst, mode = volume.split(':')
        name = dst.lower().replace('/', '-')
        if name.startswith('-'):
            name = name[1:]
        if name.endswith('-'):
            name = name[:-1]
        name = filter(lambda l: l in string.ascii_lowercase + '-', name)
        volume_args.append(
            '{name},kind=host,source={src},readOnly={ro}'.format(
                name=name,
                src=src,
                ro=str(mode == 'ro').lower()
            )
        )
        mount_args.append('volume={name},target={dst}'.format(name=name, dst=dst))
    return volume_args, mount_args


def docker_params_to_rocket_params(docker_params):
    ret = []
    for param in docker_params:
        value = None
        if param['key'] == 'memory-swap':
            pass
        elif param['key'] == 'cpu-period':
            pass
        elif param['key'] == 'cpu-quota':
            ret.append({'key': 'cpu', 'value': value})
    return ret
...
def configure_and_run_rkt_container(
        docker_client,
        docker_hash,
        service,
        instance,
...
def paasta_local_run(args):
    ...
    try:
        configure_and_run_rkt_container(
            docker_client=docker_client,
            docker_hash=tag,
            service=service,
            instance=instance,
