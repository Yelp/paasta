...
def get_flink_job_name(flink_job: FlinkJobDetails) -> str:
    """Thin wrapper for backward compat — delegates to flink_tools."""
    return flink_tools.get_flink_job_name(flink_job)
...
def _print_flink_status_from_job_manager(
    ...
    # Fetch API data (only available when cluster is running)
    flink_config = None
    overview = None
    ...
    # Collect instance details and job/pod aggregations
    try:
    ...
    # Format header (config SHA, version, dashboard URL)
    output.extend(
        flink_tools.format_flink_instance_header(instance_details, verbose > 0)
    )

    # Format verbose metadata and links
    if verbose:
        output.extend(
    ...
    # Format state and pod/job counts
    ...
    output.extend(
        flink_tools.format_flink_jobs_table(
            job_details["jobs"],
            instance_details.get("dashboard_url") or "",
            verbose,
