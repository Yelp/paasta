...
class AutoConfigUpdater:
    ...
    ) -> List[Dict[Tuple[str, str], Dict[str, Any]]]:
        merged_recs = []
        for (
            service,
            ...
            for instance_name, recommendation in recommendations_by_instance.items():
                ...
                for limit_type, limits in (
                    ...
                    log.debug(
                        f"Processing {limit_type} autotune limits for {instance_name} in {service}/{instance_type_cluster}.yaml..."
                    )
                    min_value = limits.get("min")
                    max_value = limits.get("max")
                    unclamped_resource_value = existing_recommendations[
                        instance_name
                    ].get(limit_type)
                    ...
                    if not unclamped_resource_value and min_value:
                        ...
                        log.debug(
                            f"No {limit_type} autotune data found for {instance_name} in {service}/{AUTO_SOACONFIG_SUBDIR}/{instance_type_cluster}.yaml, using autotune limit lower bound ({min_value})."
                        )
                        existing_recommendations[instance_name][limit_type] = min_value
                    ...
                    # otherwise, we can do some pretty rote clamping of resource values
                        ...
                        if min_value and unclamped_resource_value < min_value:
                            log.debug(
                                f"{limit_type} autotune config under configured limit ({min_value}) for {instance_name} in {service}/{instance_type_cluster}.yaml, using autotune limit lower bound."
                            )
                            existing_recommendations[instance_name][
                                limit_type
                            ] = min_value
                        if max_value and unclamped_resource_value > max_value:
                            log.debug(
                                f"{limit_type} autotune config over configured limit ({min_value}) for {instance_name} in {service}/{instance_type_cluster}.yaml, using autotune limit lower bound."
                            )
                            existing_recommendations[instance_name][
                                limit_type
                            ] = max_value
                        ...
                        log.debug(
                            f"No {limit_type} autotune data or limits found for {instance_name} in {service}/{instance_type_cluster}.yaml, will continue using PaaSTA defaults."
                        )

            log.info(f"Done processing {service}/{instance_type_cluster}.yaml.")
            merged_recs.append(
                {(service, instance_type_cluster): existing_recommendations}
            )
