...
"""
Usage: ./setup_namespace_services.py [options]
...
- -l <LIMIT>, --rate-limit <LIMIT>: Rate limit API calls
"""
import argparse
import logging
import os
...
def setup_namespace_services_for_kube_namespace(
    ...
    """Generate partials to reconcile Services for smartstack namespaces.
    ...
    Yields:
        Partial functions to execute service reconciliation
    """
...
def cleanup_namespace_services(
    ...
    """Generate partials to delete Services no longer in configuration.
    ...
    Yields:
        Partial functions to execute service deletion
    """
    declared_services = {
...
def namespace_operations(
    kube_client: KubeClient,
    target_namespaces: Optional[Sequence[str]],
    soa_dir: str,
    dry_run: bool,
) -> Iterator[Iterable[partial]]:
    """Generate operations to reconcile all namespace services.
    ...
    Yields:
        Iterables of partial functions to execute
    """
...
def setup_namespace_services(
    ...
    for operations in namespace_operations(
        kube_client,
        target_namespaces,
        soa_dir,
        dry_run,
        ...
        for fn in operations:
            ...
            try:
                ...
                if dry_run:
                    ...
                    log.info(
                        f"Dry run enabled: calling {func_name}{fn.args!r} with dryRun=All"
                    )

                fn()
                took = time.time() - start
                log.debug(f"Operation completed in {took}s")
            except Exception:
                success = False
