...
def sync_ssm_secrets(
    ...
    try:
        ...
        log.error("boto3 is required for syncing ssm secrets")
        return False

    config_loader = PaastaServiceConfigLoader(service=service, soa_dir=soa_dir)
    default_ssm_client = boto3.client(
        "ssm", region_name="us-west-2"
    )  # TODO: dynamic region
    sts_client = boto3.client("sts")
    ssm_clients = {None: default_ssm_client}
    ...
    for instance_type_class in K8S_INSTANCE_TYPE_CLASSES:
        for instance_config in config_loader.instance_configs(
            ...
            for ssm_secret in ssm_secrets:
                ...
                with_decryption = ssm_secret.get("with_decryption", True)
                role_arn = ssm_secret.get("assume_role_arn")

                if not source or not key:
                    log.warning(f"Invalid ssm_secret config in {service}: {ssm_secret}")
                    continue

                client = ssm_clients.get(role_arn)
                if not client:
                    try:
                        ...
                        client = boto3.client(
                            ...
                            region_name="us-east-1",  # TODO: get this from assumed role
                        )
                        ssm_clients[role_arn] = client
                ...
                try:
                    response = client.get_parameter(
                        Name=source, WithDecryption=with_decryption
                    )
            ...
            for name, secret in secret_data.items():
                create_or_update_k8s_secret(
                    ...
                    signature_name=get_ssm_secret_signature_name(
                        namespace, service, sanitise_kubernetes_name(name)
                    ),
                    secret_name=get_ssm_secret_name(
                        namespace, service, sanitise_kubernetes_name(name)
                    ),
                    get_secret_data=(lambda: {name: secret}),
                    secret_signature=_get_dict_signature({name: secret}),
                    kube_client=kube_client,
                    namespace=namespace,
                )
    return True
