...
@then(
    'we should see a job for the service "{service}" and instance "{instance}" in the job list'
)
...
def check_exit_code(context, expected_exit_code):
    try:
        assert context.exit_code == expected_exit_code, "expected %d, got %d" % (
            expected_exit_code,
            context.exit_code,
        )
...
@when(
    'we create {job_count:d} disabled jobs that look like the job stored as "{job_name}"'
)
...
@then(
    'there should be {job_count} {disabled} jobs for the service "{service}" and instance "{instance}"'
)
...
@then(
    'setup_chronos_job exits with return code "{expected_return_code}"'
    ' and the output contains "{expected_output_substring}"'
)
def check_setup_chronos_job_output(
    context, expected_return_code, expected_output_substring
):
