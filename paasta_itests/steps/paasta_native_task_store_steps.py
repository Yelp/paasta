...
def then_get_all_tasks_should_return(context, return_json):
    ...
    expected_tasks = {k: MesosTaskParameters(**v) for k, v in json.loads(return_json).items()}
...
def when_we_overwrite_task(context, task_id, params_json):
    context.task_store.overwrite_task(task_id, MesosTaskParameters(**json.loads(params_json)))
