...
def test_get_name():
    service_name = 'foo'
    actual = setup_chronos_jobs.get_name(service_name)
    expected = 'foo'
...
def test_get_memory_default():
    job_config = {}
    actual = setup_chronos_jobs.get_memory(job_config)
    expected = setup_chronos_jobs.DEFAULT_MEMORY
    assert expected == actual
...
def test_get_memory_rejects_invalid():
    ...
    try:
        setup_chronos_jobs.get_memory(job_config)
        assert False
    except ValueError:
        assert True
...
def test_parse_job_config():
    ...
    actual = setup_chronos_jobs.parse_job_config(service_name, job_config)
    expected = {
        'async': False,
        'command': '/bin/true',
