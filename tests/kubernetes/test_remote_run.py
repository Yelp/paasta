...
def test_remote_run_start():
    with patch(
        "paasta_tools.kubernetes.remote_run.KubeClient", autospec=True
    ) as mock_client, patch(
        "paasta_tools.kubernetes.remote_run.load_eks_service_config", autospec=True
    ) as mock_load_config, patch(
        "paasta_tools.kubernetes.remote_run.get_application_wrapper", autospec=True
    ) as mock_wrapper_getter:
...
def test_remote_run_start_recreate():
    with patch(
        ...
        "paasta_tools.kubernetes.remote_run.get_application_wrapper", autospec=True
    ) as mock_wrapper_getter, patch(
        "paasta_tools.kubernetes.remote_run.remote_run_stop", autospec=True
    ) as mock_stop:
...
def test_remote_run_start_toolbox():
    with patch(
        ...
        "paasta_tools.kubernetes.remote_run.generate_toolbox_deployment", autospec=True
    ) as mock_gen_config, patch(
        "paasta_tools.kubernetes.remote_run.get_application_wrapper", autospec=True
    ) as mock_wrapper_getter:
        mock_client = mock_client.return_value
...
def test_remote_run_ready():
    with patch(
        ...
        "paasta_tools.kubernetes.remote_run.find_job_pod", autospec=True
    ) as mock_find_job_pod:
        mock_client = mock_client.return_value
        mock_load_config.return_value.get_namespace.return_value = "namespace"
        mock_gen_config.return_value.get_namespace.return_value = "remote-run-toolbox"
...
def test_remote_run_token():
    with patch(
        ...
        "paasta_tools.kubernetes.remote_run.find_job_pod", autospec=True
    ) as mock_find_job_pod, patch(
        "paasta_tools.kubernetes.remote_run.create_remote_run_service_account",
        autospec=True,
    ) as mock_create_sa, patch(
        "paasta_tools.kubernetes.remote_run.create_pod_scoped_role", autospec=True
    ) as mock_create_role, patch(
        "paasta_tools.kubernetes.remote_run.bind_role_to_service_account", autospec=True
    ) as mock_bind_role, patch(
        "paasta_tools.kubernetes.remote_run.create_temp_exec_token", autospec=True
    ) as mock_create_token:
        mock_client = mock_client.return_value
        mock_load_config.return_value.get_namespace.return_value = "namespace"
        mock_job = mock_load_config.return_value.format_kubernetes_job.return_value
        mock_job.metadata.name = "somejob"
        mock_find_job_pod.return_value.metadata.name = (
            "remote-run-someuser-somejob-112233"
        )
...
def test_find_job_pod():
    with patch("paasta_tools.kubernetes.remote_run.sleep", autospec=True) as mock_sleep:
...
def test_create_remote_run_service_account():
    with patch(
        "paasta_tools.kubernetes.remote_run.get_all_service_accounts", autospec=True
    ) as mock_get_all_sa:
...
def test_generate_toolbox_deployment():
    with patch(
        "paasta_tools.kubernetes.remote_run.load_adhoc_job_config", autospec=True
    ) as mock_load_config:
        mock_load_config.return_value.config_dict = {}
        result = generate_toolbox_deployment(
            "prod-toolbox-something", "devc", "someone"
        )
