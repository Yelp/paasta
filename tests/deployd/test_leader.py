...
class TestPaastaLeaderElection(unittest.TestCase):
    def setUp(self):
        with mock.patch('paasta_tools.deployd.leader.Election.__init__', autospec=False):
            self.mock_client = mock.Mock()
            self.mock_control = mock.Mock()
            self.election = PaastaLeaderElection(self.mock_client, control=self.mock_control)
    ...
    def test_run(self):
        mock_fun = mock.Mock()
        mock_arg = mock.Mock()
        with mock.patch('paasta_tools.deployd.leader.Election.run', autospec=True) as mock_kazoo_election:
