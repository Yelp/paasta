...
@patch('paasta_tools.check_chronos_jobs.compose_monitoring_overrides_for_service', autospec=True)
...
@patch('paasta_tools.check_chronos_jobs.chronos_tools.filter_chronos_jobs', autospec=True)
...
def test_job_is_stuck_when_no_last_run(mock_chronos_client):
    assert not check_chronos_jobs.job_is_stuck(None, 2440, mock_chronos_client, 'job_name')
    mock_chronos_client.job_stat.assert_not_called()
...
def test_job_is_stuck_when_still_running(mock_chronos_client):
    ...
    interval = 60 * 60 * 24
    mock_chronos_client.job_stat.return_value = {"histogram": {"99thPercentile": 60 * 60 * 8}}
