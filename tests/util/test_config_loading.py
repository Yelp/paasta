...
from paasta_tools.util import config_loading
...
def test_deploy_whitelist_to_constraints():
    ...
    constraints = config_loading.deploy_whitelist_to_constraints(fake_whitelist)
...
def test_get_readable_files_in_glob_ignores_unreadable(tmpdir):
    ...
    ret = config_loading.get_readable_files_in_glob("*.json", tmpdir.strpath)
...
def test_load_system_paasta_config():
    json_load_return_value: config_loading.SystemPaastaConfigDict = {"cluster": "bar"}
    expected = config_loading.SystemPaastaConfig(
        json_load_return_value, "/some/fake/dir"
    )
    ...
    with mock.patch("os.path.isdir", return_value=True, autospec=True), mock.patch(
        ...
        "paasta_tools.util.config_loading.get_readable_files_in_glob",
        autospec=True,
        return_value=["/some/fake/dir/some_file.json"],
    ), mock.patch(
        "paasta_tools.util.config_loading.json.load",
        autospec=True,
        return_value=json_load_return_value,
    ) as json_patch, mock.patch(
        "paasta_tools.util.config_loading.os.stat", autospec=True
    ), mock.patch(
        "paasta_tools.util.config_loading.deep_merge_dictionaries",
        autospec=True,
        return_value=json_load_return_value,
    ) as mock_deep_merge:
        actual = config_loading.load_system_paasta_config(path="/some/fake/dir")
...
def test_load_system_paasta_config_file_non_existent_dir():
    ...
    with mock.patch("os.path.isdir", return_value=False, autospec=True):
        with raises(config_loading.PaastaNotConfiguredError) as excinfo:
            config_loading.load_system_paasta_config(fake_path)
        expected = (
...
def test_load_system_paasta_config_duplicate_keys_errors():
    ...
    with mock.patch("os.path.isdir", return_value=True, autospec=True), mock.patch(
        ...
        with raises(DuplicateKeyError):
            config_loading.load_system_paasta_config(path="/some/fake/dir")


def test_SystemPaastaConfig_get_cluster():
    fake_config = config_loading.SystemPaastaConfig(
        {"cluster": "peanut"}, "/some/fake/dir"
    )
    expected = "peanut"
...
def test_SystemPaastaConfig_get_cluster_dne():
    fake_config = config_loading.SystemPaastaConfig({}, "/some/fake/dir")
    with raises(config_loading.PaastaNotConfiguredError):
...
def test_get_service_registry():
    ...
    with mock.patch(
        ...
        actual = config_loading.get_service_docker_registry(
...
def test_get_service_registry_dne():
    ...
    fake_system_config = config_loading.SystemPaastaConfig(
...
def test_SystemPaastaConfig_get_sensu_host():
    fake_config = config_loading.SystemPaastaConfig(
        {"sensu_host": "blurp"}, "/some/fake/dir"
    )
    actual = fake_config.get_sensu_host()
    expected = "blurp"
    assert actual == expected
...
def test_SystemPaastaConfig_get_git_config(config, expected_git, expected_primary):
    fake_config = config_loading.SystemPaastaConfig(config, "/some/fake/dir")
...
def test_missing_cluster_configs_are_ignored():
    ...
    fake_system_paasta_config = config_loading.SystemPaastaConfig(
    ...
    with mock.patch(
        ...
        actual = config_loading.list_clusters(soa_dir=fake_soa_dir)
        assert actual == expected
...
def test_list_clusters_no_service_given_lists_all_of_them():
    ...
    with mock.patch(
        "paasta_tools.util.config_loading.get_soa_cluster_deploy_files",
        autospec=True,
...
def test_list_clusters_with_service():
    ...
    with mock.patch(
        ...
        actual = config_loading.list_clusters(fake_service, fake_soa_dir)
        assert actual == expected
...
def test_list_clusters_ignores_bogus_clusters():
    ...
    with mock.patch(
        ...
        actual = config_loading.list_clusters(service=fake_service)
...
def test_list_all_instances_for_service():
    ...
    with mock.patch(
        "paasta_tools.util.config_loading.list_clusters", autospec=True
    ) as mock_list_clusters, mock.patch(
        "paasta_tools.util.config_loading.get_service_instance_list", autospec=True
        ...
        actual = config_loading.list_all_instances_for_service(service)
        assert actual == expected
...
def test_get_service_instance_list():
    ...
    expected = fake_instances * len(config_loading.INSTANCE_TYPES)
    with mock.patch(
        "paasta_tools.util.config_loading.read_service_instance_names",
        autospec=True,
        return_value=fake_instances,
    ) as read_instance_names_patch:
        actual = config_loading.get_service_instance_list(
            fake_name, fake_cluster, soa_dir=fake_dir
        )
        for instance_type in config_loading.INSTANCE_TYPES:
            read_instance_names_patch.assert_any_call(
                fake_name, instance_type, fake_cluster, soa_dir=fake_dir
            )
        assert read_instance_names_patch.call_count == len(
            config_loading.INSTANCE_TYPES
        )
...
def test_read_service_instance_names_ignores_underscore():
    ...
    with mock.patch(
        ...
        actual = config_loading.read_service_instance_names(
            service=fake_name,
...
@mock.patch(
    "paasta_tools.util.config_loading.load_service_instance_auto_configs",
    autospec=True,
)
@mock.patch(
...
def test_load_service_instance_configs(
    ...
    result = config_loading.load_service_instance_configs(
        service="fake_service",
...
def test_load_service_instance_config_underscore():
    with pytest.raises(config_loading.InvalidJobNameError):
        config_loading.load_service_instance_config(
            service="fake_service",
...
def test_load_service_instance_config_not_found(mock_read_service_information):
    ...
    with pytest.raises(config_loading.NoConfigurationForServiceError):
        config_loading.load_service_instance_config(
            service="fake_service",
            instance="foo",
            instance_type="kubernetes",
...
def test_load_service_instance_config(
    ...
    result = config_loading.load_service_instance_config(
        service="fake_service",
        instance="foo",
        instance_type="kubernetes",
        cluster="fake",
...
def test_load_service_instance_auto_configs(
    ...
    result = config_loading.load_service_instance_auto_configs(
        service="fake_service",
    ...
    if instance_type_enabled:
        mock_read_extra_service_information.assert_called_with(
            "fake_service",
            f"{AUTO_SOACONFIG_SUBDIR}/marathon-fake",
            soa_dir="fake_dir",
            deepcopy=False,
        )
...
def test_get_services_for_cluster():
    ...
    with mock.patch(
        ...
        "paasta_tools.util.config_loading.get_service_instance_list",
        side_effect=lambda a, b, c, d: instances.pop(),
        autospec=True,
    ) as get_instances_patch:
        actual = config_loading.get_services_for_cluster(cluster, soa_dir=soa_dir)
...
def test_sort_dcts(dcts, expected):
    assert config_loading.sort_dicts(dcts) == expected


class TestInstanceConfig:
    def test_repr(self):
        actual = repr(
            config_loading.InstanceConfig(
                service="fakeservice",
    ...
    def test_get_cpus_in_config(self):
        fake_conf = config_loading.InstanceConfig(
            service="",
            cluster="",
            instance="",
            config_dict={"cpus": -5},
    ...
    def test_get_args_in_config_with_cmd(self):
        ...
        fake_conf.get_cmd()
        with raises(config_loading.InvalidInstanceConfig):
            fake_conf.get_args()
    ...
    def test_extra_volumes_normal(self):
        fake_extra_volumes: List[config_loading.DockerVolume] = [
    ...
    def test_get_volumes_dedupes_correctly_when_mode_differs_last_wins(self):
        ...
        system_volumes: List[config_loading.DockerVolume] = []
    ...
    def test_get_docker_url_no_error(self):
        ...
        with mock.patch(
            "paasta_tools.util.config_loading.InstanceConfig.get_docker_registry",
            autospec=True,
            return_value=fake_registry,
        ), mock.patch(
            "paasta_tools.util.config_loading.InstanceConfig.get_docker_image",
            autospec=True,
...
def test_deploy_blacklist_to_constraints():
    ...
    actual = config_loading.deploy_blacklist_to_constraints(fake_deploy_blacklist)
...
def test_validate_service_instance_valid_marathon():
    ...
    with mock.patch(
        ...
        assert (
            config_loading.validate_service_instance(
...
def test_validate_service_instance_invalid():
    ...
    with mock.patch(
        ...
        with raises(
            config_loading.NoConfigurationForServiceError,
...
def test_is_deploy_step():
    assert config_loading.is_deploy_step("prod.main")
    assert config_loading.is_deploy_step("thingy")

    assert not config_loading.is_deploy_step("itest")
    assert not config_loading.is_deploy_step("performance-check")
    assert not config_loading.is_deploy_step("command-thingy")
...
def test_get_code_sha_from_dockerurl():
    ...
    actual = config_loading.get_code_sha_from_dockerurl(fake_docker_url)
    assert actual == "git93340779"

    # Useful mostly for integration tests, where we run busybox a lot.
    assert (
        config_loading.get_code_sha_from_dockerurl("docker.io/busybox") == "gitbusybox"
    )


def test_suggest_possibilities_none():
    expected = ""
    actual = config_loading.suggest_possibilities(word="FOO", possibilities=[])
    assert actual == expected


def test_suggest_possibilities_many():
    expected = "FOOO, FOOBAR"
    actual = config_loading.suggest_possibilities(
...
def test_filter_templates_from_config_with_empty_dict():
    assert config_loading.filter_templates_from_config({}) == {}


def test_filter_templates_from_config():
    config = {"_template": "foo", "instance0": "bar", "instance1": "baz"}
    assert config_loading.filter_templates_from_config(config) == {
        "instance0": "bar",
        "instance1": "baz",
    }
