...
def mock_datetime(mock_current_time):
    with mock.patch(
        'paasta_tools.check_spark_jobs.datetime',
        autospec=True,
    ) as mock_time:
...
def mock_get_frameworks(mock_current_time):
    ...
    with mock.patch(
        'paasta_tools.check_spark_jobs.mesos_tools.get_all_frameworks',
        autospec=True,
    ) as mock_get_frameworks:
        mock_get_frameworks.return_value = [Framework(config) for config in mock_frameworks]
...
@pytest.mark.parametrize(
    'properties,expected', [
        ...
        ([['something_else', 'value'], ['spark.executorEnv.PAASTA_SERVICE', 'foo']], 'foo'),
...
def test_get_matching_framework_info(mock_guess_service, mock_get_spark_properties, mock_get_frameworks):
