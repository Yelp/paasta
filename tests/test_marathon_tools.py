...
def test_marathon_dashboard_base_url_when_there_is_only_one_dashboard():
    system_paasta_config = SystemPaastaConfig(
        config={
            'cluster': 'fake_cluster',
            'dashboard_links': {'fake_cluster': {"Marathon RO": 'http://marathon'}},
        },
    ...
    assert marathon_tools.marathon_dashboard_base_url(
        job_config=job_config,
        system_paasta_config=system_paasta_config,
    ) == 'http://marathon'


def test_marathon_dashboard_base_url_no_dashboard_links():
    system_paasta_config = SystemPaastaConfig(
        config={'cluster': 'fake_cluster'},
        directory='/fake/config',
    )
    job_config = mock.Mock(
        service='fake_service',
...
def test_marathon_dashboard_base_url_when_marathon_shard_defined():
    system_paasta_config = SystemPaastaConfig(
        config={
            'cluster': 'fake_cluster',
    ...
    job_config.get_marathon_shard.return_value = 2
    assert marathon_tools.marathon_dashboard_base_url(
        job_config=job_config,
        system_paasta_config=system_paasta_config,
    ) == 'http://marathon2'
...
def test_marathon_dashboard_base_url_when_marathon_shard_determined():
    system_paasta_config = SystemPaastaConfig(
        config={
            'cluster': 'fake_cluster',
            'dashboard_links': {'fake_cluster':
                                {"Marathon RO": ['http://marathon', 'http://marathon1']}},
        },
        directory='/fake/config',
    )
    job_config = mock.Mock(
