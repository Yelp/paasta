...
def mock_secret_provider():
    with mock.patch(
        'paasta_tools.secret_providers.vault.SecretProvider.get_vault_ecosystems_for_clusters', autospec=True,
        return_value=['devc'],
    ), mock.patch(
...
def test_get_vault_ecosystems_for_clusters(mock_secret_provider):
    ...
    assert sorted(mock_secret_provider.get_vault_ecosystems_for_clusters()) == sorted(['devc', 'prod'])
...
def test_get_secret_signature_from_data(mock_secret_provider):
    with mock.patch(
        ...
        assert not mock_secret_provider.get_secret_signature_from_data({'environments': {'devc': {}}})
