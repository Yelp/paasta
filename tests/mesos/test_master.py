...
async def test_frameworks():
    with patch.object(master.MesosMaster, '_framework_list', autospec=True) as mock_framework_list:
        fake_frameworks = [
            {
...
async def test__frameworks():
    with patch.object(master.MesosMaster, 'fetch', autospec=True) as mock_fetch:
        ...
        mock_fetch.return_value = CoroutineMock(json=CoroutineMock(return_value=mock_frameworks))
...
async def test__task_list():
    ...
    with patch.object(
        master.MesosMaster, '_framework_list', autospec=True, return_value=[mock_framework],
...
async def test_tasks():
    ...
    ) as mock__task_list, patch.object(
        task, 'Task', autospec=True,
    ) as mock_task:
