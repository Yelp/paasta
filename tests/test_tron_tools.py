...
class TestTronTools:
    ...
    def test_format_tron_action_dict_spark(self):
        ...
        assert result == {
            "command": "spark-submit --conf "
            "spark.app.name=tron_spark_my_service_my_job.do_something --conf "
            "spark.ui.port=33000 --conf "
            "spark.master=k8s://https://k8s.test-cluster.paasta:6443 --conf "
            "spark.executorEnv.PAASTA_SERVICE=my_service --conf "
            "spark.executorEnv.PAASTA_INSTANCE=my_job.do_something --conf "
            "spark.executorEnv.PAASTA_CLUSTER=test-cluster --conf "
            "spark.executorEnv.PAASTA_INSTANCE_TYPE=spark --conf "
            "spark.executorEnv.SPARK_EXECUTOR_DIRS=/tmp --conf "
            # note that the driver and executor account below are different
            "spark.kubernetes.authenticate.driver.serviceAccountName=paasta--arn-aws-iam-000000000000-role-some-role--spark "
            "--conf spark.kubernetes.pyspark.pythonVersion=3 --conf "
            "spark.kubernetes.container.image=docker-registry.com:400/my_service:paasta-123abcde "
            "--conf spark.kubernetes.namespace=paasta-spark --conf "
            # paasta workload contract labels
            "spark.kubernetes.executor.label.yelp.com/paasta_service=my_service "
            "--conf "
            "spark.kubernetes.executor.label.yelp.com/paasta_instance=my_job.do_something "
            "--conf "
            "spark.kubernetes.executor.label.yelp.com/paasta_cluster=test-cluster "
            "--conf "
            "spark.kubernetes.executor.label.paasta.yelp.com/service=my_service "
            "--conf "
            "spark.kubernetes.executor.label.paasta.yelp.com/instance=my_job.do_something "
            "--conf "
            "spark.kubernetes.executor.label.paasta.yelp.com/cluster=test-cluster "
            ...
            "--conf "
            "spark.kubernetes.executor.volumes.hostPath.1.mount.path=/etc/passwd "
            "--conf "
            "spark.kubernetes.executor.volumes.hostPath.1.options.path=/etc/passwd "
            "--conf "
            "spark.kubernetes.executor.volumes.hostPath.1.mount.readOnly=true "
            "--conf "
            "spark.kubernetes.executor.volumes.hostPath.2.mount.path=/etc/group "
            "--conf "
            "spark.kubernetes.executor.volumes.hostPath.2.options.path=/etc/group "
            "--conf "
            "spark.kubernetes.executor.volumes.hostPath.2.mount.readOnly=true "
            # user args
            "--conf spark.cores.max=4 --conf spark.driver.memory=1g --conf "
            "spark.executor.memory=1g --conf spark.executor.cores=2 --conf "
            "spark.executor.instances=2 --conf "
            "spark.kubernetes.allocation.batch.size=2 --conf "
            "spark.kubernetes.executor.limit.cores=2 --conf "
            "spark.scheduler.maxRegisteredResourcesWaitingTime=15min --conf "
            "spark.sql.shuffle.partitions=8 --conf spark.eventLog.enabled=true "
