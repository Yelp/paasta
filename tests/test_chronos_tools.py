...
class TestChronosTools:
    ...
    def test_read_chronos_jobs_for_service(self):
        ...
        with mock.patch(
            ...
            mock_load_v2_deployments_json.return_value.get_branch_dict.return_value = (
                self.fake_branch_dict
            )
            mock_read_extra_service_information.return_value = self.fake_config_file
            actual = chronos_tools.read_chronos_jobs_for_service(
                self.fake_service, self.fake_cluster, fake_soa_dir
            )
            mock_read_extra_service_information.assert_called_once_with(
                self.fake_service, expected_chronos_conf_file, soa_dir=fake_soa_dir
            )
    ...
    def test_load_chronos_job_config(self):
        ...
        with mock.patch(
            ...
            mock_load_v2_deployments_json.assert_called_once_with(
                self.fake_service, soa_dir=fake_soa_dir
            )
            mock_read_chronos_jobs_for_service.assert_called_once_with(
                self.fake_service, self.fake_cluster, soa_dir=fake_soa_dir
            )
    ...
    def test_load_chronos_job_config_unknown_job(self):
        with mock.patch(
            ...
            mock_read_chronos_jobs_for_service.assert_called_once_with(
                "fake_service", "fake_cluster", soa_dir="fake_dir"
            )
            assert (
                str(exc.value)
                == 'No job named "fake_job" in config file chronos-fake_cluster.yaml'
            )
    ...
    def test_get_epsilon(self):
        ...
        fake_conf = chronos_tools.ChronosJobConfig(
            ...
            cluster="fake_cluster",
            instance="fake_instance",
            config_dict={"epsilon": fake_epsilon},
            branch_dict=None,
        )
        actual = fake_conf.get_epsilon()
    ...
    def test_format_chronos_job_dict_uses_time_parser(self, system_paasta_config):
        ...
        with mock.patch(
            "paasta_tools.chronos_tools.parse_time_variables",
            autospec=True,
    ...
    def test_get_calculated_constraints_respects_constraints_override(
    ...
    def test_get_calculated_constraints_includes_system_blacklist(self):
        ...
        fake_system_paasta_config = SystemPaastaConfig(
            {"deploy_blacklist": [["region", "foo"]]}, "/foo"
        )
        actual = fake_conf.get_calculated_constraints(
            system_paasta_config=fake_system_paasta_config
        )
        assert sorted(actual) == sorted(
            [["pool", "LIKE", "poolname"], ["region", "UNLIKE", "foo"]]
        )
    ...
    def test_get_calculated_constraints_includes_system_whitelist(self):
        ...
        fake_system_paasta_config = SystemPaastaConfig(
            {"deploy_whitelist": ["region", ["foo"]]}, "/foo"
        )
        actual = fake_conf.get_calculated_constraints(
            system_paasta_config=fake_system_paasta_config
        )
        assert sorted(actual) == sorted(
            [["pool", "LIKE", "poolname"], ["region", "LIKE", "foo"]]
        )
    ...
    def test_get_schedule_interval_in_seconds(self):
        ...
        assert (
            fake_conf.get_schedule_interval_in_seconds() == 60 * 60 * 24
        )  # once a day
    ...
    def test_get_schedule_interval_in_seconds_if_monthly_crontab_format(self):
        ...
        assert (
            60 * 60 * 24 * 28
            <= fake_conf.get_schedule_interval_in_seconds()
            <= 60 * 60 * 24 * 31
        )

    def test_get_schedule_interval_in_seconds_if_weekly_crontab_format(self):
        fake_schedule = "0 14 * * 4"
        fake_conf = chronos_tools.ChronosJobConfig(
            service="fake_name",
            cluster="fake_cluster",
            instance="fake_instance",
        ...
        assert 60 * 60 * 24 * 7 == fake_conf.get_schedule_interval_in_seconds()
    ...
    def test_check_parents_one_bad(self):
        ...
        assert (
            msg
            == "The job name(s) service1-instance1 is not formatted correctly: expected service.instance"
        )
    ...
    def test_check_bounce_method_invalid(self):
        ...
        assert msg.startswith(
            'The specified bounce method "crossover" is invalid. It must be one of ('
        )
    ...
    def test_check_disk_invalid(self):
        ...
        assert okay is False
        assert (
            msg == 'The specified disk value "all of it" is not a valid float or int.'
        )

    def test_check_schedule_repeat_helper_valid(self):
        assert (
            self.fake_invalid_chronos_job_config._check_schedule_repeat_helper("R32")
            is True
        )

    def test_check_schedule_repeat_helper_valid_infinite(self):
        assert (
            self.fake_invalid_chronos_job_config._check_schedule_repeat_helper("R")
            is True
        )

    def test_check_schedule_repeat_helper_invalid_empty(self):
        assert (
            self.fake_invalid_chronos_job_config._check_schedule_repeat_helper("")
            is False
        )

    def test_check_schedule_repeat_helper_invalid_no_r(self):
        assert (
            self.fake_invalid_chronos_job_config._check_schedule_repeat_helper("32")
            is False
        )

    def test_check_schedule_repeat_helper_invalid_float(self):
        assert (
            self.fake_invalid_chronos_job_config._check_schedule_repeat_helper("R6.9")
            is False
        )

    def test_check_schedule_repeat_helper_invalid_negative(self):
        assert (
            self.fake_invalid_chronos_job_config._check_schedule_repeat_helper("R-8")
            is False
        )

    def test_check_schedule_repeat_helper_invalid_other_text(self):
        assert (
            self.fake_invalid_chronos_job_config._check_schedule_repeat_helper("BR72")
            is False
        )
    ...
    def test_check_schedule_invalid_start_time_no_t_designator(self):
        ...
        fake_isodate_exception = "ISO 8601 time designator 'T' missing. Unable to parse datetime string {!r}".format(
            "now"
        )
        okay, msg = chronos_config.check_schedule()
        assert okay is False
    ...
    def test_check_schedule_invalid_start_time_bad_date(self):
        ...
        fake_isodate_exception = "Unrecognised ISO 8601 date format: {!r}".format(
            "today"
        )
        okay, msg = chronos_config.check_schedule()
        assert okay is False
    ...
    def test_check_schedule_invalid_start_time_bad_time(self):
        ...
        fake_isodate_exception = "Unrecognised ISO 8601 time format: {!r}".format(
            "morning"
        )
        okay, msg = chronos_config.check_schedule()
        assert okay is False
    ...
    def test_check_schedule_low_interval(self):
        ...
        assert (
            msg
            == 'Unsupported interval "PT10S": jobs must be run at an interval of > 60 seconds'
        )
    ...
    def test_get_chronos_jobs_for_cluster(self):
        with mock.patch(
            ...
            assert (
                chronos_tools.get_chronos_jobs_for_cluster(
                    "mycluster", soa_dir="my_soa_dir"
                )
                == []
            )
            get_services_for_cluster_patch.assert_called_once_with(
                "mycluster", "chronos", "my_soa_dir"
            )
    ...
    def test_lookup_chronos_jobs_with_service_and_instance(self):
        ...
        with mock.patch(
            "paasta_tools.chronos_tools.filter_chronos_jobs", autospec=True
        ) as mock_filter_chronos_jobs:
            chronos_tools.lookup_chronos_jobs(
                client=fake_client, service=fake_service, instance=fake_instance
            )
    ...
    def test_get_chronos_status_for_job(self):
        ...
        fake_csv = [
            ...
            "node,{},fresh,{}".format(
                chronos_tools.compose_job_id(fake_service, fake_instance),
                expected_status,
            ),
        ...
        status = chronos_tools.get_chronos_status_for_job(
            fake_client, fake_service, fake_instance
        )
    ...
    def test_filter_chronos_jobs_with_no_filters(self):
        fake_jobs = [
            {
                "name": chronos_tools.compose_job_id("fake_service", "fake_instance"),
                "disabled": False,
            },
            {
                "name": chronos_tools.compose_job_id(
                    "other_fake_service", "other_fake_instance"
                ),
                "disabled": False,
            },
        ]
    ...
    def test_filter_chronos_jobs_include_disabled(self):
        ...
        fake_jobs = [
            ...
            {
                "name": chronos_tools.compose_job_id(
                    "some_other_service", "some_other_instance"
                ),
                "disabled": False,
            },
        ]
    ...
    def test_filter_chronos_jobs_skips_non_paasta_job_id(self):
        fake_jobs = [{"name": "some non-paasta job", "disabled": False}]
        actual = chronos_tools.filter_chronos_jobs(
            jobs=fake_jobs,
            service="whatever",
            instance="whatever",
    ...
    def test_create_complete_config(self, system_paasta_config):
        ...
        with mock.patch(
            ...
            "paasta_tools.chronos_tools.get_config_hash",
            return_value=fake_config_hash,
            autospec=True,
            ...
            load_system_paasta_config_patch.return_value.get_volumes = mock.Mock(
                return_value=[]
            )
            load_system_paasta_config_patch.return_value.get_dockercfg_location = mock.Mock(
            ...
            mock_get_config_hash.assert_called_with(
                config=mock_get_config_for_bounce_hash.return_value, force_bounce=None
            )
            expected = {
                "arguments": None,
    ...
    def test_create_complete_config_understands_parents(self, system_paasta_config):
        ...
        with mock.patch(
            ...
            "paasta_tools.chronos_tools.load_chronos_job_config",
            autospec=True,
            return_value=self.fake_dependent_chronos_job_config,
        ), mock.patch(
            "paasta_tools.utils.get_service_docker_registry",
            return_value=fake_registry,
            autospec=True,
    ...
    def test_create_complete_config_considers_disabled(self, system_paasta_config):
        ...
        with mock.patch(
            ...
            first_description = chronos_tools.create_complete_config(
                "fake-service", "fake-job"
            )["description"]
            ...
            second_description = chronos_tools.create_complete_config(
                "fake-service", "fake-job"
            )["description"]
    ...
    def test_create_complete_config_desired_state_start(self, system_paasta_config):
        ...
        with mock.patch(
            ...
            "paasta_tools.chronos_tools.load_chronos_job_config",
            autospec=True,
            return_value=fake_chronos_job_config,
        ), mock.patch(
            "paasta_tools.utils.get_service_docker_registry",
            return_value=fake_registry,
            autospec=True,
    ...
    def test_create_complete_config_respects_extra_volumes(self, system_paasta_config):
        fake_owner = "test@test.com"
        fake_extra_volumes = [
            {"containerPath": "/extra", "hostPath": "/extra", "mode": "RO"}
        ]
        fake_system_volumes = [
            {"containerPath": "/system", "hostPath": "/system", "mode": "RO"}
        ]
        ...
        with mock.patch(
            ...
            load_system_paasta_config_patch.return_value.get_volumes = mock.Mock(
                return_value=fake_system_volumes
            )
            load_system_paasta_config_patch.return_value.get_dockercfg_location = mock.Mock(
                return_value="file:///root/.dockercfg"
            )
    ...
    def test_wait_for_job(self):
        ...
        with mock.patch(
            "paasta_tools.chronos_tools.sleep", autospec=True
        ) as mock_sleep:
    ...
    def test_filter_enabled_jobs(self):
        ...
        assert chronos_tools.filter_enabled_jobs(fake_jobs) == [
            {"name": "foo", "disabled": False}
        ]
    ...
    def test_filter_non_temporary_jobs(self):
        ...
        expected = [{"name": "example_service mesosstage_robjreruntest"}]
    ...
    def test__get_related_jobs_and_configs_only_independent_jobs(
        ...
        jobs, configs = _get_related_jobs_and_configs(
            cluster=self.fake_cluster, ttl=-1
        )  # ttl=-1 to disable cache

        expected_jobs = {
            (self.fake_service, self.fake_job_name): {
                (self.fake_service, self.fake_job_name)
            },
            (self.fake_service, self.fake_invalid_job_name): {
                (self.fake_service, self.fake_invalid_job_name)
            },
        }
        expected_configs = {}
    ...
    def test__get_related_jobs_and_configs_only_independent_jobs_skips_non_deployed_services(
        ...
        mock_load_v2_deployments_json.return_value.get_branch_dict.side_effect = (
    ...
    def test__get_related_jobs_and_configs_with_dependent_jobs(
        ...
        related_jobs = {
            (self.fake_service, self.fake_job_name),
            (self.fake_service, self.fake_dependent_job_name),
        }
        expected_jobs = {
    ...
    def test_get_config_for_bounce_hash(self):
        with mock.patch(
            ...
            mock_config = {"thing": "blah", "environmentVariables": []}
            ret = get_config_for_bounce_hash(
                mock_config, "service1", "/nail/blah", mock_system_config
            )
