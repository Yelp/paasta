...
def test_stop_or_start_warn_on_multi_instance(
    ...
    assert "please specify --allow-multiple-services for this command to take affect" in out
...
@mock.patch('paasta_tools.cli.cmds.start_stop_restart.utils.get_git_url', autospec=True)
@mock.patch('paasta_tools.cli.cmds.start_stop_restart.list_clusters', autospec=True)
def test_stop_or_start_warn_but_allow_on_multi_instance(
    mock_list_clusters,
    mock_get_git_url,
    mock_get_instance_config,
    mock_instance_config,
    ...
    args, _ = parse_args([
        'start', '-s', 'fake_service,other_service',
        '-c', 'cluster1,cluster2', '-d', '/soa/dir', '--allow-multiple-services',
    ])
    mock_list_clusters.return_value = ['cluster1', 'cluster2']
    mock_get_git_url.return_value = 'fake_git_url'
    mock_get_instance_config.return_value = mock_instance_config
    ...
    assert ret == 0
    assert "Warning: trying to start/stop/restart multiple services" in out
    assert "--allow-multiple-services specified, continuing anyways" in out
