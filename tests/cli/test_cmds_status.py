...
def make_fake_instance_conf(cluster, service, instance, deploy_group=None, team=None, registrations=()):
    ...
    if registrations is None:
        del conf.get_registrations  # http://www.voidspace.org.uk/python/mock/mock.html#deleting-attributes
    return conf
...
def test_paasta_status(system_paasta_config):
    marathon_status = Struct(
        ...
        app_statuses=[],
        mesos=Struct(running_task_count=2, error_message=None, running_tasks=[], non_running_tasks=[]),
        smartstack=Struct(
    ...
    fake_status_obj = Struct(git_sha="fake_git_sha", instance="fake_instance", service="fake_service")
