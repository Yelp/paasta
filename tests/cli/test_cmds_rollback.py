...
def test_paasta_rollback_mark_for_deployment_simple_invocation():
    with patch(
        ...
        "paasta_tools.cli.cmds.rollback.get_versions_for_service", autospec=True
    ) as mock_get_versions_for_service, patch(
        "paasta_tools.cli.cmds.rollback.mark_for_deployment", autospec=True
    ) as mock_mark_for_deployment, patch(
        "paasta_tools.cli.cmds.rollback.get_git_url", autospec=True
    ) as mock_get_git_url, patch(
        "paasta_tools.cli.cmds.rollback.figure_out_service_name", autospec=True
        ...
        "paasta_tools.cli.cmds.rollback.get_currently_deployed_version", autospec=True
    ) as mock_get_currently_deployed_version:
        ...
        for call_args in mock_log_audit.call_args_list:
            ...
            assert (
                call_kwargs["action_details"]["deploy_group"] in fake_args.deploy_groups
            )
...
def test_paasta_rollback_mark_for_deployment_with_image():
    with patch(
        ...
        for call_args in mock_log_audit.call_args_list:
            ...
            assert call_kwargs["action_details"]["rolled_back_to"] == str(
                rollback_version
            )
            assert (
                call_kwargs["action_details"]["rollback_type"]
...
def test_paasta_rollback_mark_for_deployment_wrong_deploy_group_args():
    with patch(
        ...
        mock_figure_out_service_name.return_value = fake_args.service
        mock_list_deploy_groups.return_value = [
            "some_other_instance.some_other_cluster"
        ]
