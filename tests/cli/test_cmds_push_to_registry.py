...
def test_build_command():
    with patch(
        "paasta_tools.cli.cmds.push_to_registry.build_docker_tag", autospec=True
    ) as mock_build_docker_tag:
        mock_build_docker_tag.return_value = (
...
def test_push_to_registry_run_fail():
    with patch(
        "paasta_tools.cli.cmds.push_to_registry._log_audit", autospec=True
    ) as mock_log_audit, patch(
        "paasta_tools.cli.cmds.push_to_registry._log", autospec=True
    ), patch(
        "paasta_tools.cli.cmds.push_to_registry._run", autospec=True
    ) as mock_run, patch(
        "paasta_tools.cli.cmds.push_to_registry.validate_service_name", autospec=True
    ), patch(
        "paasta_tools.cli.cmds.push_to_registry.build_command", autospec=True
    ) as mock_build_command, patch(
        "paasta_tools.cli.cmds.push_to_registry.is_docker_image_already_in_registry",
        autospec=True,
    ) as mock_is_docker_image_already_in_registry:
...
def test_is_docker_image_already_in_registry_success():
    with patch(
        "paasta_tools.cli.cmds.push_to_registry.read_docker_registry_creds",
        autospec=True,
    ) as mock_read_docker_registry_creds, patch(
        "paasta_tools.cli.cmds.push_to_registry.requests.Session.head", autospec=True
    ) as mock_request_head, patch(
        "paasta_tools.cli.cmds.push_to_registry.get_service_docker_registry",
        autospec=True,
...
def test_push_to_registry_with_image_version():
    with patch(
        "paasta_tools.cli.cmds.push_to_registry._log_audit", autospec=True
    ), patch("paasta_tools.cli.cmds.push_to_registry._log", autospec=True), patch(
        "paasta_tools.cli.cmds.push_to_registry._run", autospec=True
    ) as mock_run, patch(
        "paasta_tools.cli.cmds.push_to_registry.validate_service_name", autospec=True
    ), patch(
        ...
        "paasta_tools.cli.cmds.push_to_registry.retag_versioned_image", autospec=True
    ) as mock_retag_versioned_image:
        args, _ = parse_args(
            [
                "push-to-registry",
                "-s",
        ...
        mock_retag_versioned_image.assert_called_once_with(
            "foo", "abcd" * 10, "extrastuff"
        )
...
def test_retag_versioned_image():
    with patch(
        "paasta_tools.cli.cmds.push_to_registry._run", autospec=True
    ) as mock_run, patch(
        "paasta_tools.cli.cmds.push_to_registry.build_docker_tag", autospec=True
