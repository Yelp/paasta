...
def test_paasta_remote_run_start():
    with patch(
        ...
        "paasta_tools.cli.cmds.remote_run.run_interactive_cli", autospec=True
    ) as mock_run_cli, patch(
        "paasta_tools.cli.cmds.remote_run.shutil", autospec=True
    ) as mock_shutil, patch(
        "paasta_tools.cli.cmds.remote_run.time", autospec=True
    ) as mock_time, patch(
        ...
        mock_client.remote_run.remote_run_token.return_value = MagicMock(
            token="aaabbbccc"
        )
...
def test_paasta_remote_run_stop():
    with patch(
        "paasta_tools.cli.cmds.remote_run.get_paasta_oapi_client_with_auth",
        autospec=True,
    ) as mock_get_client, patch(
...
def test_paasta_remote_run_copy():
    with patch(
        "paasta_tools.cli.cmds.remote_run.load_eks_or_adhoc_deployment_config",
        autospec=True,
    ) as mock_load_eks, patch(
        "paasta_tools.cli.cmds.remote_run.subprocess", autospec=True
