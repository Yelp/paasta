...
def test_check_if_instance_is_done(side_effect, expected):
    with patch(
...
def test_wait_for_deployment():
    with patch(
        ...
        autospec=True,
    ) as mock_check_if_instance_is_done, patch(
        "paasta_tools.cli.cmds.mark_for_deployment._log", autospec=True
    ), patch(
        "paasta_tools.cli.cmds.mark_for_deployment.get_instance_configs_for_service_in_deploy_group_all_clusters",
        autospec=True,
    ) as mock_get_instance_configs_for_service_in_deploy_group_all_clusters, patch(
        ...
        mock_check_if_instance_is_done.side_effect = (
            check_if_instance_is_done_side_effect
        )
...
def test_wait_for_deployment_raise_no_such_cluster():
    with patch("paasta_tools.cli.cmds.mark_for_deployment._log", autospec=True), patch(
        "paasta_tools.cli.cmds.mark_for_deployment.PaastaServiceConfigLoader",
        autospec=True,
    ) as mock_paasta_service_config_loader, patch(
        "paasta_tools.cli.cmds.mark_for_deployment.load_system_paasta_config",
        autospec=True,
    ) as mock_load_system_paasta_config:
        mock_load_system_paasta_config.return_value.get_api_endpoints.return_value = {
...
def test_paasta_wait_for_deployment_return_1_when_no_such_service():
    with patch(
        ...
        "paasta_tools.cli.cmds.wait_for_deployment.validate_service_name", autospec=True
...
def test_paasta_wait_for_deployment_return_1_when_deploy_group_not_found():
    with patch(
        ...
        "paasta_tools.cli.cmds.wait_for_deployment.list_deploy_groups", autospec=True
...
def test_paasta_wait_for_deployment_return_0_when_no_instances_in_deploy_group(
    ...
    with patch(
        ...
        "paasta_tools.cli.cmds.wait_for_deployment.validate_git_sha", autospec=True
    ) as mock_validate_git_sha, patch(
        "paasta_tools.cli.cmds.wait_for_deployment.validate_service_name", autospec=True
...
def test_get_latest_marked_version_good():
    with patch(
        "paasta_tools.cli.cmds.wait_for_deployment.list_remote_refs", autospec=True
...
def test_get_latest_marked_version_with_image_good():
    with patch(
        "paasta_tools.cli.cmds.wait_for_deployment.list_remote_refs", autospec=True
    ) as mock_list_remote_refs:
        mock_list_remote_refs.return_value = {
        ...
        assert get_latest_marked_version("", "fake_group1") == DeploymentVersion(
            sha="968b948b3fca457326718dc7b2e278f89ccc5c87",
            image_version="20161128image",
        )


def test_get_latest_marked_version_bad():
    with patch(
        "paasta_tools.cli.cmds.wait_for_deployment.list_remote_refs", autospec=True
    ) as mock_list_remote_refs:
        mock_list_remote_refs.return_value = {
...
def test_validate_deploy_group_when_is_git_not_available(capsys):
    with patch(
        "paasta_tools.cli.cmds.wait_for_deployment.list_remote_refs", autospec=True
    ) as mock_list_remote_refs:
