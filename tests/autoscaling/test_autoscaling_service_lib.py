...
def test_get_service_metrics_provider():
    assert (
        autoscaling_service_lib.get_service_metrics_provider("mesos_cpu")
...
def test_get_decision_policy():
    assert (
        autoscaling_service_lib.get_decision_policy("proportional")
        == autoscaling_service_lib.proportional_decision_policy
    )  # NOQA


def test_threshold_decision_policy():
    decision_policy_args = {"threshold": 0.1, "current_instances": 10}
    with mock.patch(
        ...
        assert (
            autoscaling_service_lib.threshold_decision_policy(
                error=0, **decision_policy_args
            )
            == 0
        )
        assert (
            autoscaling_service_lib.threshold_decision_policy(
                error=0.5, **decision_policy_args
            )
            == 1
        )
        assert (
...
def test_mesos_cpu_metrics_provider():
    ...
    fake_mesos_task_3 = mock.MagicMock(stats=asynctest.CoroutineMock(return_value={}))
    fake_mesos_task.__getitem__.return_value = "fake-service.fake-instance"
        ...
        return_value=mock.Mock(
            get=mock.Mock(
                side_effect=lambda x: (
                    str(zookeeper_get_payload[x.split("/")[-1]]).encode("utf-8"),
                    None,
                )
            )
        ),
...
def test_autoscale_marathon_instance():
    ...
    with mock.patch(
        ...
        "paasta_tools.autoscaling.autoscaling_service_lib.get_service_metrics_provider",
        autospec=True,
        **{"return_value.return_value": 0},
        ...
        "paasta_tools.autoscaling.autoscaling_service_lib.get_decision_policy",
        autospec=True,
    ...
    with mock.patch(
        ...
        **{"return_value.return_value": 0},
        ...
        mock_set_instances_for_marathon_service.assert_called_once_with(
            service="fake-service",
            instance="fake-instance",
            instance_count=int(current_instances * 0.7),
        )
...
def test_autoscale_marathon_instance_aborts_when_wrong_number_tasks():
    ...
    with mock.patch(
        ...
        "paasta_tools.autoscaling.autoscaling_service_lib.is_task_data_insufficient",
        autospec=True,
...
def test_autoscale_services_happy_path():
    ...
    with mock.patch(
        ...
        "paasta_tools.autoscaling.autoscaling_service_lib.get_marathon_clients",
        autospec=True,
    ), mock.patch(
        "paasta_tools.autoscaling.autoscaling_service_lib.get_marathon_servers",
        autospec=True,
    ), mock.patch(
        "paasta_tools.autoscaling.autoscaling_service_lib.get_marathon_apps_with_clients",
        autospec=True,
        ...
        "paasta_tools.autoscaling.autoscaling_service_lib.load_system_paasta_config",
        autospec=True,
        ...
        "paasta_tools.autoscaling.autoscaling_service_lib.PaastaServiceConfigLoader.instance_configs",
        autospec=True,
        ...
        mock_format_marathon_app_dict.return_value = {
            "id": "fake-service.fake-instance.sha123.sha456"
        }
...
def test_autoscale_services_not_healthy():
    ...
    with mock.patch(
        ...
        "paasta_tools.autoscaling.autoscaling_service_lib.is_old_task_missing_healthchecks",
        autospec=True,
...
def test_filter_autoscaling_tasks():
    ...
    with mock.patch(
        ...
        assert ret == (
            {
                "fake-service.fake-instance.sha123.sha456.uuid": mock_marathon_app.tasks[
                    0
                ]
            },
            mock_mesos_tasks,
...
def test_get_new_instance_count():
    with mock.patch(
        ...
        "paasta_tools.autoscaling.autoscaling_service_lib.compose_autoscaling_zookeeper_root",
        autospec=True,
...
def test_get_autoscaling_info():
    with mock.patch(
        "paasta_tools.autoscaling.autoscaling_service_lib.get_utilization",
        autospec=True,
    ) as mock_get_utilization, mock.patch(
        "paasta_tools.autoscaling.autoscaling_service_lib.filter_autoscaling_tasks",
        autospec=True,
    ) as mock_filter_autoscaling_tasks, mock.patch(
        "paasta_tools.autoscaling.autoscaling_service_lib.get_error_from_utilization",
        autospec=True,
        ...
        mock_apps_with_clients = [
            (mock.Mock(name="fake_app"), mock.Mock(name="fake_client"))
        ]
        ...
        mock_service_config = mock.Mock(
            ...
            get_autoscaling_params=mock.Mock(
                return_value={"myarg": "param", "setpoint": 0.7}
            ),
            get_instances=mock.Mock(return_value=4),
        )
        mock_marathon_task = mock.Mock()
        mock_mesos_tasks = mock.Mock()
        mock_filter_autoscaling_tasks.return_value = (
            {"id1": mock_marathon_task},
            mock_mesos_tasks,
        )
        ...
        mock_get_utilization.assert_called_with(
            ...
            autoscaling_params={"myarg": "param", "noop": True, "setpoint": 0.7},
            log_utilization_data={},
...
def test_serialize_and_deserialize_historical_load():
    ...
    assert (
        len(serialized) == 50 * autoscaling_service_lib.SIZE_PER_HISTORICAL_LOAD_RECORD
    )
...
def test_serialize_historical_load_trims_oldest_data():
    ...
    deserialized_long = autoscaling_service_lib.deserialize_historical_load(
        serialized_long
    )
...
def test_proportional_decision_policy(mock_fetch_historical_load):
    ...
    assert 0 == autoscaling_service_lib.proportional_decision_policy(
        setpoint=0.5, utilization=0.476, **common_kwargs  # Just over 0.475 = 0.5 * 0.95
    )

    assert 1 == autoscaling_service_lib.proportional_decision_policy(
        setpoint=0.5, utilization=0.526, **common_kwargs  # Just over 0.525 = 0.5 * 1.05
    )
    ...
    assert 5 == autoscaling_service_lib.proportional_decision_policy(
        setpoint=0.5, utilization=0.75, **common_kwargs
    )

    # If we're 50% underutilized, scale down by 50%
    assert -5 == autoscaling_service_lib.proportional_decision_policy(
        setpoint=0.5, utilization=0.25, **common_kwargs
    )
        ...
        mock.Mock(
            tasks=[
                mock.Mock(
                    ...
                    app_id="service.instance.gitOLD.configOLD",
                ),
                mock.Mock(
        ...
        mock.Mock(
            tasks=[
                mock.Mock(
                    ...
                    app_id="service.instance.gitNEW.configNEW",
                ),
                mock.Mock(
